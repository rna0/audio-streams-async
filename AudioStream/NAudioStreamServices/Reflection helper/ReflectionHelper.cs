...
namespace AudioStream.NAudioStreamServices.Reflection_helper
{
    ...
    public static class ReflectionHelper
    {
        ...
        private static readonly Dictionary<Type, Func<object>> _defaultConstructorsDictionary =
            new Dictionary<Type, Func<object>>();

        private static readonly Func<Type, Func<object>> _getDefaultConstructorDelegate =
            GetDefaultConstructorDelegate<object>;
        ...
        {
            var result = Ext.GetOrCreateValue(_defaultConstructorsDictionary, ref _defaultConstructorsLock, objectType,
                _getDefaultConstructorDelegate);
            return result;
        }

        private static YieldReaderWriterLockSlim _typedDefaultConstructorsLock;

        private static readonly Dictionary<KeyValuePair<Type, Type>, Delegate> _typedDefaultConstructorsDictionary =
            new Dictionary<KeyValuePair<Type, Type>, Delegate>();

        private static readonly Func<KeyValuePair<Type, Type>, Delegate> _getTypedDefaultConstructorDelegate =
            _GetDefaultConstructorDelegate;
        ...
        public static Func<T> GetDefaultConstructorDelegate<T>(Type objectType)
        {
            ...
            var result = Ext.GetOrCreateValue(_typedDefaultConstructorsDictionary, ref _typedDefaultConstructorsLock,
                pair, _getTypedDefaultConstructorDelegate);
        ...
        private static readonly Dictionary<KeyValuePair<ConstructorInfo, Type>, Delegate> _typedConstructors =
            new Dictionary<KeyValuePair<ConstructorInfo, Type>, Delegate>();

        private static readonly Func<KeyValuePair<ConstructorInfo, Type>, Delegate> _getTypedConstructorDelegate =
        ...
        public static Delegate GetConstructorDelegate(ConstructorInfo constructor, Type delegateType)
        {
            ...
            var result = Ext.GetOrCreateValue(_typedConstructors, ref _getTypedConstructorLock, pair,
                _getTypedConstructorDelegate);
            return result;
        }
        ...
        private static Delegate _GetConstructorDelegate(KeyValuePair<ConstructorInfo, Type> pair)
        {
            ...
            if (invokeMethod == null)
                throw new InvalidOperationException(
                    "The given delegate type does not have an Invoke method. Is this a compilation error?");
            ...
            if (constructorParameterTypes.Length != count)
                throw new InvalidOperationException(
                    "The number of parameters between the constructor and the delegate is not compatible.");

            var parameterExpressions = new ParameterExpression[count];
            var arguments = new Expression[count];
            for (int i = 0; i < count; i++)
            {
                var argument = _GetArgumentExpression(i, constructorParameterTypes, invokeParameterTypes,
        ...
        private static readonly Dictionary<ConstructorInfo, FastDynamicDelegate> _constructors =
            new Dictionary<ConstructorInfo, FastDynamicDelegate>();

        private static readonly Func<ConstructorInfo, FastDynamicDelegate> _getConstructorDelegate =
        ...
        public static FastDynamicDelegate GetConstructorDelegate(ConstructorInfo constructor)
        {
            ...
            var result = Ext.GetOrCreateValue(_constructors, ref _constructorsLock, constructor,
                _getConstructorDelegate);
            return result;
        }
        ...
        private static FastDynamicDelegate _CreateConstructorDelegate(ConstructorInfo constructor)
        {
            ...
            if (count != 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    Expression accessParameterExpression =
                        Expression.ArrayAccess(parametersExpression, constantExpression);
                    ...
                    if (parameterType.IsByRef)
                    {
                        ...
                        if (parameterType != typeof(object))
                        {
                            ...
                            if (!parameter.IsOut)
                            {
                                ...
                                if (parameterType != typeof(object))
                                    effectiveAccessParameterExpression =
                                        Expression.Convert(accessParameterExpression, parameterType);
        ...
        private static readonly Dictionary<MethodInfo, FastMethodCallDelegate> _methodsDictionary =
        ...
        private static FastMethodCallDelegate _CreateMethodCallDelegate(MethodInfo method)
        {
            ...
            if (method.ReturnType == typeof(void))
            {
                var returnExpression = Expression.Return(returnTarget, Expression.Constant(null, typeof(object)),
                    typeof(object));
                instructions.Add(returnExpression);
            }
            else
        ...
        private static readonly Dictionary<KeyValuePair<MethodInfo, Type>, Delegate> _getTypedMethodCallDictionary =
            new Dictionary<KeyValuePair<MethodInfo, Type>, Delegate>();

        private static readonly Func<KeyValuePair<MethodInfo, Type>, Delegate> _getTypedMethodCallDelegate =
            _GetTypedMethodCallDelegate;
        ...
        public static Delegate GetMethodCallDelegate(MethodInfo method, Type delegateType)
        {
            ...
            var result = Ext.GetOrCreateValue(_getTypedMethodCallDictionary, ref _getTypedMethodCallLock, pair,
                _getTypedMethodCallDelegate);
            return result;
        }
        ...
        private static Delegate _GetTypedMethodCallDelegate(KeyValuePair<MethodInfo, Type> pair)
        {
            ...
            {
                var argument = _GetArgumentExpression(i + startIndex, methodParameterTypes, invokeParameterTypes,
                    parameterExpressions);
                arguments[i] = argument;
            }
            ...
            else
            {
                var instanceExpression =
                    _GetArgumentExpression(0, methodParameterTypes, invokeParameterTypes, parameterExpressions);
        ...
        private static Expression _GetArgumentExpression(int index, IList<Type> methodParameterTypes,
            Type[] invokeParameterTypes, ParameterExpression[] parameterExpressions)
        {
        ...
        private static readonly Dictionary<PropertyInfo, Func<object, object>> _getPropertiesDictionary =
            new Dictionary<PropertyInfo, Func<object, object>>();

        private static readonly Func<PropertyInfo, Func<object, object>> _getPropertyGetterDelegate =
            GetPropertyGetterDelegate<object, object>;
        ...
        {
            var result = Ext.GetOrCreateValue(_getPropertiesDictionary, ref _getPropertyLock, property,
                _getPropertyGetterDelegate);
            return result;
        }

        private static YieldReaderWriterLockSlim _getTypedPropertyLock;

        private static readonly Dictionary<KeyValuePair<Type, PropertyInfo>, Delegate> _getTypedPropertiesDictionary =
            new Dictionary<KeyValuePair<Type, PropertyInfo>, Delegate>();

        private static readonly Func<KeyValuePair<Type, PropertyInfo>, Delegate> _getTypedPropertyGetterDelegate =
            _GetPropertyGetterDelegate;
        ...
        public static Func<TInstance, TOutput> GetPropertyGetterDelegate<TInstance, TOutput>(PropertyInfo property)
        {
            ...
            var result = Ext.GetOrCreateValue(_getTypedPropertiesDictionary, ref _getTypedPropertyLock, pair,
                _getTypedPropertyGetterDelegate);
        ...
        private static readonly Dictionary<PropertyInfo, Action<object, object>> _setPropertiesDictionary =
            new Dictionary<PropertyInfo, Action<object, object>>();

        private static readonly Func<PropertyInfo, Action<object, object>> _getPropertySetterDelegate =
            GetPropertySetterDelegate<object, object>;
        ...
        {
            var result = Ext.GetOrCreateValue(_setPropertiesDictionary, ref _setPropertyLock, property,
                _getPropertySetterDelegate);
            return result;
        }

        private static YieldReaderWriterLockSlim _typedPropertySetterDelegatesLock;

        private static readonly Dictionary<KeyValuePair<Type, PropertyInfo>, Delegate>
            _typedPropertySetterDelegatesDictionary = new Dictionary<KeyValuePair<Type, PropertyInfo>, Delegate>();

        private static Func<KeyValuePair<Type, PropertyInfo>, Delegate> _typedGetPropertySetterDelegate =
            _TypedGetPropertySetterDelegate;
        ...
        public static Action<TInstance, TValue> GetPropertySetterDelegate<TInstance, TValue>(PropertyInfo property)
        {
            ...
            var result = Ext.GetOrCreateValue(_typedPropertySetterDelegatesDictionary,
                ref _typedPropertySetterDelegatesLock, pair, _typedGetPropertySetterDelegate);
        ...
        private static readonly Dictionary<EventInfo, Action<object, Delegate>> _addEventDictionary =
            new Dictionary<EventInfo, Action<object, Delegate>>();

        private static readonly Func<EventInfo, Action<object, Delegate>> _getEventAdderDelegate =
            GetEventAdderDelegate<object, Delegate>;
        ...
        {
            var result =
                Ext.GetOrCreateValue(_addEventDictionary, ref _addEventLock, eventInfo, _getEventAdderDelegate);
            return result;
        }
        ...
        private static readonly Dictionary<EventInfo, Action<object, Delegate>> _removeEventDictionary =
            new Dictionary<EventInfo, Action<object, Delegate>>();

        private static readonly Func<EventInfo, Action<object, Delegate>> _getEventRemoverDelegate =
            GetEventRemoverDelegate<object, Delegate>;
        ...
        {
            var result = Ext.GetOrCreateValue(_removeEventDictionary, ref _removeEventLock, eventInfo,
                _getEventRemoverDelegate);
            return result;
        }
        ...
        private static Action<TInstance, TDelegate> _GetEventDelegate<TInstance, TDelegate>(MethodInfo method,
            Type handlerType)
        {
            ...
            var lambda =
                Expression.Lambda<Action<TInstance, TDelegate>>(callExpression, instanceParameter, handlerParameter);
            var result = lambda.Compile();
            return result;
        }
        ...
        private static readonly Dictionary<FieldInfo, Func<object, object>> _getFieldsDictionary =
            new Dictionary<FieldInfo, Func<object, object>>();

        private static readonly Func<FieldInfo, Func<object, object>> _getFieldGetterDelegate =
            GetFieldGetterDelegate<object, object>;
        ...
        private static readonly Dictionary<KeyValuePair<Type, FieldInfo>, Delegate> _getTypedFieldsDictionary =
            new Dictionary<KeyValuePair<Type, FieldInfo>, Delegate>();

        private static readonly Func<KeyValuePair<Type, FieldInfo>, Delegate> _getTypedFieldGetterDelegate =
            _GetFieldGetterDelegate;
        ...
        public static Func<TInstance, TOutput> GetFieldGetterDelegate<TInstance, TOutput>(FieldInfo field)
        {
            ...
            var result = Ext.GetOrCreateValue(_getTypedFieldsDictionary, ref _getTypedFieldLock, pair,
                _getTypedFieldGetterDelegate);
            return (Func<TInstance, TOutput>) result;
        }
        ...
        {
            public static TValue GetOrCreateValue<TKey, TValue>(Dictionary<TKey, TValue> dictionary,
                ref YieldReaderWriterLockSlim readerWriterLock, TKey key, Func<TKey, TValue> createValue)
            {
                TValue result;
            ...
            private static readonly Dictionary<MethodBase, Type[]> _parameterTypes =
                new Dictionary<MethodBase, Type[]>();
